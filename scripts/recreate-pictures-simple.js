...
// Функция для создания картины БЕЗ imageUrl (пока что)
  ...
  try {
    const response = await axios.post(
      `${CONFIG.API_BASE_URL}/pictures`,
      {
        ...
        // Пока не передаем imageUrl, так как сервер не поддерживает
      },
      {
        headers: {
          'Authorization': `Bearer ${CONFIG.AUTH_TOKEN}`,
...
// Основная функция
async function main() {
  console.log('🚀 Начинаем пересоздание картин...\n');
  ...
  console.log('\n💡 Следующие шаги:');
  console.log('1. Обновить сервер с поддержкой imageUrl');
  console.log('2. Обновить картины с изображениями');
  console.log('3. Настроить фронтенд для отображения картин');
}
