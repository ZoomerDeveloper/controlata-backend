...
async function checkPictureSizes() {
  try {
    ...
    if (pictureSizes.length === 0) {
      ...
      const basicSizes = [
        { name: '20x30 см', width: 20, height: 30, isActive: true },
        { name: '30x40 см', width: 30, height: 40, isActive: true },
        { name: '40x50 см', width: 40, height: 50, isActive: true },
        { name: '50x70 см', width: 50, height: 70, isActive: true }
      ];
      ...
      pictureSizes.forEach(size => {
        console.log(`  - ${size.name} (${size.width}x${size.height} см) [ID: ${size.id}]`);
      });
    }
