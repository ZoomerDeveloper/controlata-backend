...
function extractRussianPictures(data) {
  console.log('🔍 Структура данных:', JSON.stringify(data, null, 2).substring(0, 1000) + '...');
  ...
  console.log('🔍 Количество предложений:', offers ? offers.length : 'undefined');
  
  if (!offers) {
    console.error('Предложения не найдены');
    return [];
  }
  ...
  for (const offer of offers) {
    if (offer.categoryId && offer.categoryId[0] === CONFIG.RUSSIAN_CATEGORY_ID) {
      russianPictures.push({
        id: offer.$.id,
        name: offer.name ? offer.name[0] : 'Без названия',
        vendorCode: offer.vendorCode ? offer.vendorCode[0] : 'Без кода',
        picture: offer.picture ? offer.picture[0] : '',
        url: offer.url ? offer.url[0] : '',
        price: offer.price ? parseFloat(offer.price[0]) : 0,
        currencyId: offer.currencyId ? offer.currencyId[0] : 'EUR',
        dimensions: offer.dimensions ? offer.dimensions[0] : '1/50/40'
      });
    }
  }
  ...
  try {
    const response = await axios.post(
      `${CONFIG.API_BASE_URL}/pictures`,
      {
        ...
        imageUrl: picture.picture // Используем изображение из каталога
        // orderId не передаем для готовых картин
      },
      {
        headers: {
    ...
    console.error(`Полный ответ сервера:`, error.response?.status, error.response?.statusText);
...
async function importPictures() {
  ...
  const xmlData = await parseXMLFile();
  if (!xmlData) {
    console.error('❌ Не удалось распарсить XML файл');
    return;
  }

  // Извлекаем русские картины
  console.log('🔍 Извлекаем русские картины...');
  const russianPictures = extractRussianPictures(xmlData);
...
module.exports = { importPictures, extractRussianPictures, parseXMLFile };
