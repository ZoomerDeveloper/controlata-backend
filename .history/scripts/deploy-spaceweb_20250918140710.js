...
async function deployToSpaceWeb() {
  ...
  try {
    ...
    try {
      // Очищаем только папку public_html
      await client.removeDir('/public_html');
    } catch (error) {
      console.log('ℹ️ Папка public_html уже пуста или не существует');
    }

    console.log('📤 Загружаем файлы в public_html...');
    await client.uploadFromDir(BUILD_DIR, '/public_html');
    ...
    console.log('🌐 Ваш сайт доступен по адресу: http://admin-art24.online.swtest.ru');
