...
function parseYAMLFile() {
  try {
    ...
    console.error('Ошибка парсинга YAML файла:', error);
    return null;
  }
}
...
function extractRussianPictures(data) {
  if (!data || !data.yml_catalog || !data.yml_catalog.shop || !data.yml_catalog.shop.offers) {
    console.error('Неверная структура YAML файла');
    return [];
  }

  const offers = data.yml_catalog.shop.offers;
  const russianPictures = [];

  for (const offer of offers) {
    if (offer.categoryId === CONFIG.RUSSIAN_CATEGORY_ID) {
