...
// Функция для создания картины с изображением в заметках
  ...
  try {
    const response = await axios.post(
      `${CONFIG.API_BASE_URL}/pictures`,
      {
        ...
        notes: `URL: ${picture.url}\nРазмеры: ${picture.dimensions}\n🖼️ ИЗОБРАЖЕНИЕ: ${picture.picture}`
        // Помещаем URL изображения в заметки с эмодзи для легкого поиска
      },
      {
        headers: {
          'Authorization': `Bearer ${CONFIG.AUTH_TOKEN}`,
...
// Основная функция
async function main() {
  console.log('🚀 Начинаем создание картин с изображениями в заметках...\n');
  ...
  russianPictures.slice(0, 5).forEach((picture, index) => {
    ...
    console.log(`   Изображение: ${picture.picture}`);
  });
  console.log('...\n');
  ...
  console.log('\n💡 Теперь фронтенд может извлекать изображения из заметок!');
}
