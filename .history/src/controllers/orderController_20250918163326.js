      ...
      for (const pictureData of pictures) {
        ...
        if (pictureData.type === 'READY_MADE' && pictureData.pictureId) {
          ...
          if (existingPicture) {
            ...
            // Копируем материалы и списываем их со склада
              ...
              for (const pictureMaterial of existingPicture.materials) {
                ...
                try {
                  ...
                  const result = await warehouseService.removeMaterialFromStock(
                    ...
                    `Создание заказа: ${order.orderNumber}`,
                    order.id,
                    'ORDER',
                    `Заказ: ${order.orderNumber}, Картина: ${existingPicture.name}`
                  );

                  if (result.isNegative) {
                    logger.warn('Material stock went negative during order creation', {
                      materialId: pictureMaterial.materialId,
                      quantity: pictureMaterial.quantity,
                  ...
                  logger.error('Error removing material from stock during order creation', {
                    materialId: pictureMaterial.materialId,
                    quantity: pictureMaterial.quantity,
                    orderId: order.id,
                    pictureId: createdPicture.id,
          ...
          // Для картин по фото материалы нужно будет добавить отдельно
          // через интерфейс управления картинами
        }
      }
    }
