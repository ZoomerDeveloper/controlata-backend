...
const createOrderSchema = Joi.object({
  ...
  pictures: Joi.array().items(
    Joi.object({
      ...
      pictureSizeId: Joi.string().when('type', {
        is: 'CUSTOM_PHOTO',
        then: Joi.required(),
        otherwise: Joi.optional()
      }),
      name: Joi.string().min(1).max(200).when('type', {
        is: 'CUSTOM_PHOTO',
        then: Joi.required(),
        otherwise: Joi.optional()
      }),
      ...
      if (value.type === 'READY_MADE' && !value.pictureId) {
        return helpers.error('any.required', { label: 'pictureId' });
      }
      if (value.type === 'CUSTOM_PHOTO' && !value.name) {
