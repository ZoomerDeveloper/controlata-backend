...
class ApiService {
  ...
  async getNotifications(params?: {
    ...
    severity?: string;
    type?: string;
  }): Promise<{ notifications: any[]; stats: any; pagination: any }> {
  ...
  async checkLowStock(): Promise<any> {
    const response = await this.api.post('/notifications/check-low-stock');
    return response.data;
  }

  async cleanupOldNotifications(): Promise<any> {
    const response = await this.api.delete('/notifications/cleanup');
    return response.data;
  }
  ...
  async getCustomerById(customerId: string): Promise<{ customer: any }> {
  ...
  async getCustomerStats(): Promise<{ stats: any }> {
    const response = await this.api.get('/customers/stats');
    return response.data;
  }

  // Upload API
  async uploadPicture(file: File): Promise<{ url: string }> {
    const formData = new FormData();
    formData.append('image', file);
    const response = await this.api.post('/upload/picture', formData, {
      headers: {
