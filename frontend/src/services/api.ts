...
class ApiService {
  ...
  constructor() {
    this.api = axios.create({
      baseURL: process.env.REACT_APP_API_URL || 'https://controlata-production.up.railway.app/api',
      timeout: 10000,
      headers: {
  ...
  async getLowStockMaterials(): Promise<{ lowStockMaterials: Stock[] }> {
    const response = await this.api.get('/materials/low-stock/list');
    return response.data;
  }
